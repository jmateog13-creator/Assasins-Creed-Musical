...
const game = {
            ...
            setTimeout(() => {
                content.innerHTML += `
                    <div class="feedback-panel success">
                        ...
                        <button class="action-btn" onclick="game.startRhythmGame()">CONTINUAR INVESTIGACIÓ</button>
                    </div>
                `;
            }, 1000);
        } else {
                    ...
                    content.innerHTML += `
                        <div class="feedback-panel error">
                            <div class="warning-panel">
                                ...
                                <p><em>"Res és veritat, tot està permès... excepte la ignorància musical."</em></p>
                            </div>
    ...
    // Iniciar joc de ritme (pregunta secundària)
    startRhythmGame() {
        const chapter = gameData.chapters[gameData.currentChapter];
        const content = document.getElementById('gameContent');
            ...
            <div class="memory-panel">
                ...
                <div class="memory-content">
                    <div class="challenge-section">
                        ...
                        <div class="options-grid" id="rhythmOptions">
                            ${chapter.rhythmGame.options.map((option, index) => 
                                `<button class="option-btn" onclick="game.answerRhythm(${index})">${option}</button>`
                            ).join('')}
                        </div>
                    </div>
                </div>
    ...
    // Respondre pregunta de ritme
    answerRhythm(selectedAnswer) {
        const chapter = gameData.chapters[gameData.currentChapter];
        const buttons = document.querySelectorAll('#rhythmOptions .option-btn');
        const content = document.getElementById('gameContent');
            ...
            setTimeout(() => {
                content.innerHTML += `
                    <div class="feedback-panel success">
                        ...
                        <p>Els secrets rítmics d'aquesta època són vostres. Abstergo no podrà destruir aquesta informació.</p>
                ...
                setTimeout(() => {
                    content.innerHTML += `
                        <div class="feedback-panel error">
                            ...
                            <button class="action-btn" onclick="game.startRhythmGame()">REINTENTAR</button>
                        </div>
                    `;
                }, 1000);
            }
    ...
    saveGame() {
        if (typeof(Storage) !== "undefined") {
            ...
            localStorage.setItem('animus_fragments_save', JSON.stringify(saveData));
        }
    },
    ...
    loadGame() {
        if (typeof(Storage) !== "undefined") {
            const saved = localStorage.getItem('animus_fragments_save');
            if (saved) {
