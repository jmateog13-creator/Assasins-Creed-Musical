...
const game = {
            ...
            setTimeout(() => {
                content.innerHTML += `
                    <div class="feedback-panel success">
                        ...
                        <button class="action-btn" onclick="game.startMelodyGame()">CONTINUAR INVESTIGACIÓ</button>
                    </div>
                `;
            }, 1000);
        } else {
                    ...
                    content.innerHTML += `
                        <div class="feedback-panel error">
                            <div class="warning-panel">
                                ...
                                <p><em>"Res és veritat, tot està permès... excepte la ignorància melòdica."</em></p>
                            </div>
    ...
    // Iniciar joc de melodia (pregunta secundària)
    startMelodyGame() {
        const chapter = gameData.chapters[gameData.currentChapter];
        const content = document.getElementById('gameContent');
            ...
            <div class="memory-panel">
                ...
                <div class="memory-content">
                    <div class="challenge-section">
                        ...
                        <div class="options-grid" id="melodyOptions">
                            ${chapter.rhythmGame.options.map((option, index) => 
                                `<button class="option-btn" onclick="game.answerMelody(${index})">${option}</button>`
                            ).join('')}
                        </div>
                    </div>
                </div>
    ...
    // Respondre pregunta de melodia
    answerMelody(selectedAnswer) {
        const chapter = gameData.chapters[gameData.currentChapter];
        const buttons = document.querySelectorAll('#melodyOptions .option-btn');
        const content = document.getElementById('gameContent');
            ...
            setTimeout(() => {
                content.innerHTML += `
                    <div class="feedback-panel success">
                        ...
                        <p>Els secrets melòdics d'aquesta època són vostres. Abstergo no podrà destruir aquesta informació.</p>
                ...
                setTimeout(() => {
                    content.innerHTML += `
                        <div class="feedback-panel error">
                            ...
                            <button class="action-btn" onclick="game.startMelodyGame()">REINTENTAR</button>
                        </div>
                    `;
                }, 1000);
            }
    ...
    saveGame() {
        if (typeof(Storage) !== "undefined") {
            ...
            localStorage.setItem('animus_melody_save', JSON.stringify(saveData));
        }
    },
    ...
    loadGame() {
        if (typeof(Storage) !== "undefined") {
            const saved = localStorage.getItem('animus_melody_save');
            if (saved) {
